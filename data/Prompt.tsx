...
- Theme usage:
  - Use CSS variables style tokens:
    ...
    var(--primary), var(--muted-foreground), etc.

- Mention gradient strategy (subtle background gradients, card gradients, glow highlights)
...
- If included, it MUST be explicit and detailed:
  ...
  - Style: glassmorphism backdrop-blur-md, bg opacity, border,
    rounded-3xl, shadow

  - List EXACT 5 icons by name
    (e.g., lucide:home, lucide:compass, lucide:zap,
...
- ACTIVE MAPPING guideline:
  ...
  - Track → Primary action / Workflow screen
    (e.g., Workout, Create, Scan)
  - Profile → Settings / Account
  - Menu → More / Extras

- IMPORTANT:
  - Do NOT write bottom nav as a lazy copy for every screen.
  - Icons can stay consistent, but the ACTIVE icon MUST change
...
- Include explicit navigation details in layoutDescription:
  - Header height, sticky behavior, search placement,
    user menu, notifications
  - Sidebar width, collapsed state, active link styling,
    section grouping

- If a dashboard:
  - Include breadcrumb + page title area

- Use lucide icons for nav items
- Show active state styling
  (bg-[var(--muted)] or border-l-2 border-[var(--primary)], etc.)
